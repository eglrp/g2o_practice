...
void BuildProblem(const BALProblem* bal_problem, g2o::SparseOptimizer* optimizer, const BundleParams& params)
{
    ...
    //raw_cameras是位姿顶点的起始地址
    const double* raw_cameras = bal_problem->cameras();
    for(int i = 0; i < num_cameras; ++i)
    {
        //raw_cameras是起始地址,camera_block_size是camera的大小,
    ...
    //raw_points是路标顶点的起始地址
    const double* raw_points = bal_problem->points();
    // const int point_block_size = bal_problem->point_block_size();
    for(int j = 0; j < num_points; ++j)
    {
        //raw_points是起始地址,point_block_size是point的大小,
    ...
    for(int i = 0; i < num_observations; ++i)
    {
        ...
        //camera_id和point_id是不能重复的
        //camera_index()是一组数,存的是数据集中的第一列的数,所以说他的大小是观测的数量
        //point_index()是一组数,存的是数据集中的第二列的数,所以说他的大小是观测的数量
...
void WriteToBALProblem(BALProblem* bal_problem, g2o::SparseOptimizer* optimizer)
{
    ...
    for(int i = 0; i < num_cameras; ++i)
    {
        ...
        //raw_cameras是起始地址 camera_block_size是相机变量的大小,NewCameraVec.data()是src起始地址
